...
setTimeout(() => {
                     ...
                     setTimeout(() => {
                     m.reply('```Mengumpulkan Hasil Buruan🕸️```')
                     }, 15000) 
                    
                     setTimeout(() => {
                     m.reply('```Menembak Target Dengan Senapan M16☢️```')
                     }, 14000) 
                     
                     setTimeout(() => {
                     m.reply('```Mencari Hewan Buruan🔎```')
                     }, 0) 
  user.as = new Date * 1
    } else conn.sendBut(m.chat, `Stamina Kamu Kurang\nSilahkan Istirahat Dulu Untuk Melanjutkan Berburu\n🕖 *${timers}*`, wm, 'Inventory', '.inv',m )
}
