...
let handler  = async (m, { conn, usedPrefix }) => {
  ...
  let id = m.chat
  if (id in global.kuis) return conn.reply(m.chat, '*Tolong Selesaikan Soal Ini Terlebih Dahulu*', global.kuis[id][0])
  global.kuis[id] = [
    await conn.reply(m.chat,`*❮ KUIS ❯*\n\n📑Pertanyaan :\n*${pickRandom(global.kuis)}*\n\n⏱️Waktu : 30.00 Detik\n🎁Bonus : 5000 XP`, m),
    kuis, 4,
    setTimeout(() => {
      if (global.kuis[id]) conn.sendBut(m.chat, `⏱️ Waktu Habis!\nCoba Lagi Lain Waktu`, wm, 'Kuis', '.kuis', m)
