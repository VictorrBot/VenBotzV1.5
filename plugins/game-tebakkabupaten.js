...
let handler = async (m, { conn, usedPrefix }) => {
  ...
  if (id in conn.tebakkabupaten) {
    conn.reply(m.chat, '*Tolong Selesaikan Soal Ini Terlebih Dahulu*', conn.tebakkabupaten[id][0])
    throw false
  }
...
🎁Bonus: ${poin} XP
  ...
  conn.tebakkabupaten[id] = [
    await conn.sendButtonImg(m.chat, await (await fetch(json.url)).buffer(), caption, '', '🔍 HELP', '.tebu', m)
    ,
    json, poin,
    setTimeout(async () => {
      if (conn.tebakkabupaten[id]) await conn.sendBut(m.chat, `「⏱️」Waktu habis!\nJawabannya adalah *${json.title}*`, '', 'Tebak Kabupaten', '.tebakkabupaten', conn.tebakkabupaten[id][0])
